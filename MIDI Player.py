                    ...
                    case 15:    # Meta Events
                        ...
                        match meta_type:
                            ...
                            case 0x51:  # Set Tempo
                                ...
                                # only update tick for PPQ (not SMPTE header mode)
                                if not (get_bit(MIDI[12], 7)):
                                    tick = Tempo / 1000000 / Divisions
                                track_events.append((event, event2, total_delta, 0x51, Tempo))
                            case 0x54:  # SMPTE Offset
                                # Parse 5-byte SMPTE offset: hourByte, minute, second, frame, fraction
                                hour_byte = MIDI[i]
...
# Calculate the standard tick utilizing either standard timing, or the SMPTE timecode.
Tempo = 500000

SMPTE_offset = 0
if get_bit(MIDI[12], 7):
    fps = - int.from_bytes(MIDI[12], 'big', signed=True)
    ticks_per_frame = int.from_bytes(MIDI[13], 'big')
    tick = 1 / (fps * ticks_per_frame)
else:
    Divisions = int.from_bytes(MIDI[12:14], 'big')
    tick = Tempo / 1000000 / Divisions
...
for pos in Track_Pos:
    ...
    while i < track_end:
        ...
        while True:
            ...
            if (byte & 0b10000000) == 0:
                break
        total_delta += delta*tick + SMPTE_offset
        if (MIDI[i] & 0b10000000):
            status = MIDI[i]
            i += 1
        event = (status & 0b11110000) >> 4
                    ...
                    case 15:    # Meta Events
                        ...
                        match meta_type:
                            ...
                            case 0x54:  # SMPTE Offset
                                ...
                                hours = hour_byte & 0x1F
                                fps_map = {0:24, 1:25, 2:30000/1001, 3:30}
                                fps = fps_map[(hour_byte >> 5) & 0x03]
                                minutes, seconds = MIDI[i+1], MIDI[i+2]
                                frames, fraction = MIDI[i+3], MIDI[i+4]
                                SMPTE_offset = hours * 3600 + minutes * 60 + seconds + frames/fps + fraction/(fps*100)
                            case 0x58:
                                Time_Signature = MIDI[i], MIDI[i+1]
                                track_events.append((event, event2, total_delta, meta_type, Time_Signature))
                        i += length
